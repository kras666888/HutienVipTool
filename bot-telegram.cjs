...
function loadStateFromFile() {
  try {
    ...
    console.error("Load tx2 stats failed:", err?.message || err);
  }
}
...
function writeStatsFile(entry) {
  ...
  try {
    ...
    console.error("Write tx2 stats failed:", err?.message || err);
  }
}
...
async function run() {
  ...
  const base = "https://taixiu2.apiquadautayshelby.vip/signalr";
  const hub = "luckydiceMd5Hub";
  ...
  ws.on("message", async (data) => {
    try {
      ...
      for (const item of msg.M) {
        if (item.M !== "sessionInfo") continue;
        ...
        if (typeof d === "object" && d.Dice1 === -1) continue;
        if (d === -1) continue;

        if (lastResultSession === session) continue;
